    ...
    () {
      ...
      test(
        ...
        () async {
          ...
          container.read(internetConnectionCheckerPod).onStatusChange.listen((event) {});

          expect(status, InternetStatus.connected);
          expect(container.read(internetConnectionCheckerPod).checkInterval,
              equals(const Duration(seconds: 5)));
          container.read(internetCheckerPod.notifier).change(status: InternetStatus.disconnected);
          expect(
              container.read(internetCheckerPod).requireValue, equals(InternetStatus.disconnected));
        },
      );
      test(
